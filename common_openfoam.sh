...
{
    export DEST_DIR="$FOAM_RUN/$MODEL_NAME"_x
    ...
    cp case_full/system/blockMeshDict $DEST_DIR/system/blockMeshDict
    cp case_full_x/system/forceCoeffs $DEST_DIR/system/forceCoeffs
    cp $MODEL_FILE $DEST_DIR/constant/triSurface/model.stl
    cd $DEST_DIR
    ./run.sh
    cd $FOAM_RUN
    printf "%d\t" $1 &>> forceCoeffs_$MODEL_NAME.dat
    tail -n 1 "$MODEL_NAME"_x/postProcessing/forceCoeffs1/0/forceCoeffs.dat &>> forceCoeffs_$MODEL_NAME.dat
}
...
{
    export DEST_DIR="$FOAM_RUN/$MODEL_NAME"_y
    ...
    cd $FOAM_RUN
    printf "%d\t" $1 &>> forceCoeffs_$MODEL_NAME.dat
    tail -n 1 "$MODEL_NAME"_y/postProcessing/forceCoeffs1/0/forceCoeffs.dat &>> forceCoeffs_$MODEL_NAME.dat
}
...
{
    export DEST_DIR="$FOAM_RUN/$MODEL_NAME"_z
    ...
    cd $FOAM_RUN
    printf "%d\t" $1 &>> forceCoeffs_$MODEL_NAME.dat
    tail -n 1 "$MODEL_NAME"_z/postProcessing/forceCoeffs1/0/forceCoeffs.dat &>> forceCoeffs_$MODEL_NAME.dat
}
